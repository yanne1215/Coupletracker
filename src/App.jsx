...
const db = {
  async getExpenses() {
    ...
    return (await r.json()).map(r => ({
      ...
      currency: r.currency || "CAD", split: r.split || "equal",
...
function splitLabel(sp) {
  return sp==="equal"?"Split equally":sp==="yanne"?"Yanne's expense":sp==="tim"?"Tim's expense":"Split equally";
}
...
export default function App() {
  ...
  const EMPTY = {date:today,store:"",category:"groceries",amount:"",note:"",payer:"yanne",split:"equal",paymentMethod:"Credit Card",currency:"CAD",photo:null,isSettlement:false};
  const [form,setForm] = useState(EMPTY);

  // Load
  useEffect(()=>{
    Promise.all([db.getExpenses(),db.getSetting("categories"),db.getSetting("stores"),db.getSetting("payMethods"),db.getSetting("currency")])
  ...
  const saveSetting = async (key,val)=>{ try{ await db.setSetting(key,val); }catch{} };

  // ── Shared AI call ──────────────────────────────────────────────────────────
  const callAI = async (file, prompt, maxTokens=800) => {
  ...
  const handlePhoto = useCallback(async (file,setter)=>{
    ...
    try {
      const text = await callAI(file,
        `Analyze this receipt image. Respond ONLY with a JSON object, no markdown, no extra text:\n{"amount":number,"store":"exact store name (for Canadian stores identify: Loblaws,No Frills,Metro,Sobeys,Costco,Walmart,Tim Hortons,McDonald's,A&W,Harvey's,The Keg,Canadian Tire,Home Depot,IKEA,Shoppers Drug Mart,Petro-Canada,Esso,Cineplex,Sport Chek,LCBO,Beer Store,Best Buy,Dollarama)","category":"one of [${catList}]","note":"under 40 chars","currency":"CAD|USD|EUR|GBP|HKD|TWD|JPY|AUD"}`,
        600
      );
      const p = extractJSON(text);
      setter(f=>({...f,amount:p.amount||"",store:p.store||"",category:p.category||"other",note:p.note||"",currency:p.currency||f.currency,photo:URL.createObjectURL(file)}));
      showToast("Receipt scanned!");
    } catch {
      setter(f=>({...f,photo:URL.createObjectURL(file)}));
  ...
  const handleBulkScan = useCallback(async (file)=>{
    ...
    try {
      const text = await callAI(file,
        `This image shows a bank statement or list of transactions. Extract every debit/purchase transaction you can see.\nRespond ONLY with a valid JSON array, no markdown fences, no extra text:\n[{"date":"${nowYear}-MM-DD","amount":number,"store":"merchant name","category":"one of [${catList}]","note":"brief description under 30 chars","currency":"CAD|USD|EUR|GBP|HKD|TWD|JPY|AUD"}]\nRules:\n- Positive amounts only (debits/purchases)\n- Skip credits, refunds, transfers to self\n- If no year visible assume ${nowYear}\n- If no transactions found return empty array []`,
        3000
      );
      ...
      setBulkItems(parsed.map((t,i)=>({
        ...
        date: t.date||today,
        payer:"yanne", split:"equal", paymentMethod:"Credit Card", selected:true,
      })));
    } catch(err) {
      setBulkError("Could not read transactions. Make sure the screenshot shows transaction amounts and dates clearly.");
      setBulkItems([]);
    }
    setIsAnalyzing(false);
  },[categories]);
  ...
  const handleBulkSave = async ()=>{
    ...
    setSyncing(true);
    try {
      const rows = toSave.map(t=>({date:t.date,store:t.store||"",category:t.category||"other",amount:t.amount,note:t.note||"",payer:t.payer,paymentMethod:t.paymentMethod,currency:t.currency||currency,split:t.split}));
      const saved = await db.insertMany(rows);
      setRecords(prev=>[...saved.map((r,i)=>({...rows[i],id:r.id,isSettlement:false,photo:null})),...prev]);
      setBulkItems(null);
      closeModal();
  ...
  const handleSave = async ()=>{
    ...
    try {
      ...
      setRecords(r=>[{...newRec,id},...r]);
      setForm(EMPTY); setView("home"); showToast("Expense saved");
    } catch { showToast("Failed to save","err"); }
    setSyncing(false);
  };
  ...
  const handleSettle = async ()=>{
    ...
    const pLabel=allNet>0?"Tim":"Yanne", rLabel=allNet>0?"Yanne":"Tim";
    setSyncing(true);
    const s={date:today,store:"",category:"other",amount:absNet,note:`${pLabel} paid ${rLabel} — settled up`,payer,split:"equal",paymentMethod:"E-Transfer",currency,isSettlement:true,photo:null};
    try {
      const id=await db.insertExpense(s);
      setRecords(r=>[{...s,id},...r]);
      closeModal(); showToast(`Settled! ${pLabel} paid ${rLabel} ${fmt(absNet,currency)}`);
    } catch { showToast("Failed","err"); }
    setSyncing(false);
  };

  // ── Settings helpers ────────────────────────────────────────────────────────
  const updateCategories = async (arr)=>{ const s=sortCatsAZ(arr); setCategories(s); await saveSetting("categories",s); };
  const updateStores     = async (arr)=>{ const s=sortAZ(arr);     setStores(s);     await saveSetting("stores",s);     };
  const updatePayMethods = async (arr)=>{ const s=sortAZ(arr);     setPayMethods(s); await saveSetting("payMethods",s); };
  const updateCurrency   = async (c)  =>{ setCurrency(c);                            await saveSetting("currency",c);   };
  ...
  const catStats = categories.map(c=>({...c,total:filtered.filter(r=>!r.isSettlement&&r.category===c.id).reduce((s,r)=>s+r.amount,0),count:filtered.filter(r=>!r.isSettlement&&r.category===c.id).length})).filter(c=>c.total>0).sort((a,b)=>b.total-a.total);
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .upload-btn{border:1.5px dashed #2A2A2A;border-radius:12px;background:#161616;display:flex;align-items:center;justify-content:center;gap:8px;height:48px;cursor:pointer;font-size:13px;color:#555;font-weight:500;width:100%;}
    ...
    .bulk-item{background:#161616;border:1px solid #2A2A2A;border-radius:12px;padding:12px 14px;margin-bottom:10px;}
    .bulk-item.on{border-color:#6366F1;background:#1A1833;}
    .chk{width:22px;height:22px;border-radius:6px;border:2px solid #333;display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;transition:all 0.15s;}
    .chk.on{background:#6366F1;border-color:#6366F1;}
    /* ── NAV: fixed height, flex column centred, equal widths ── */
    .nav-bar{position:fixed;bottom:0;left:0;right:0;background:#111111;border-top:1px solid #1E1E1E;display:flex;height:58px;}
    ...
    .nav-icon{font-size:18px;font-weight:700;line-height:1;display:block;}
    .nav-icon.lg{font-size:22px;}
    .nav-label{font-size:10px;line-height:1;display:block;}
  `;
  ...
  return (
    <div style={{minHeight:"100vh",background:S.bg,fontFamily:"'DM Sans',sans-serif",color:S.text}}>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div style={{textAlign:"center",paddingBottom:20}}>
              ...
              {Math.abs(allNet)<0.01
                ?<div style={{color:S.muted2,fontSize:14}}>Already settled!</div>
                :<>
                  <div style={{background:"#0D1117",border:`1px solid ${S.border2}`,borderRadius:14,padding:"16px",marginBottom:20}}>
      ...
      {modal==="bulkReview"&&(
        <div className="overlay">
          <div className="modal">
            ...
            <div style={{fontSize:12,color:S.muted,marginBottom:14}}>Tap row to toggle · edit fields inline</div>

            {isAnalyzing&&(
              <div style={{textAlign:"center",padding:"50px 0",color:S.muted}}>
                <div style={{fontSize:36,marginBottom:14}}>🔍</div>
                <div style={{fontSize:15,fontWeight:500}}>Scanning statement...</div>
                <div style={{fontSize:12,marginTop:6,color:S.muted}}>This may take a few seconds</div>
              </div>
            )}

            {!isAnalyzing&&bulkError&&(
              <div style={{background:"#3B0000",border:"1px solid #7F1D1D",borderRadius:12,padding:"16px",marginBottom:16}}>
                <div style={{fontWeight:600,color:"#FCA5A5",marginBottom:6}}>⚠️ Could not extract transactions</div>
                <div style={{fontSize:12,color:"#FCA5A5",opacity:0.8,marginBottom:14}}>{bulkError}</div>
                <div style={{fontSize:12,color:"#FCA5A5",opacity:0.7}}>Tips: Use a clear screenshot showing transaction dates and amounts. Crop out unrelated content.</div>
                <label className="upload-btn" style={{marginTop:12,borderColor:"#7F1D1D",color:"#FCA5A5"}}>
                  <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{if(e.target.files[0])handleBulkScan(e.target.files[0]);}}/>
                  Try another screenshot
                </label>
              </div>
            )}

            {!isAnalyzing&&bulkItems&&bulkItems.length>0&&(
              <>
                ...
                <button className="btn" onClick={handleBulkSave} disabled={syncing} style={{width:"100%",marginTop:4,padding:"15px",background:syncing?"#3730A3":"#6366F1",borderRadius:13,fontSize:15,fontWeight:700,color:"#fff"}}>
                  {syncing?"Saving...":bulkItems.filter(i=>i.selected).length>0?`Save ${bulkItems.filter(i=>i.selected).length} Expenses`:"Select at least one"}
                </button>
              </>
            )}
          </div>
      ...
      {modal==="editRecord"&&modalData&&(
        <EditRecordModal record={modalData} categories={categories} stores={stores} payMethods={payMethods} onSave={handleEditSave} onDelete={()=>handleDelete(modalData.id)} onClose={closeModal} isAnalyzing={isAnalyzing} onPhoto={f=>handlePhoto(f,u=>setModalData(d=>({...d,...u})))} editFileRef={editFileRef} S={S}/>
      )}
      ...
      {modal==="currency"&&(
        <div className="overlay" onClick={closeModal}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {CURRENCIES.map(c=>(
              <button key={c.code} className="btn" onClick={()=>{updateCurrency(c.code);closeModal();}}
      ...
      {(modal==="addCat"||modal==="editCat")&&(
        <EditItemModal
          ...
          onSave={async v=>{
            ...
            const newCats=modal==="addCat"?[...categories,{id,label:v.label.trim(),icon:v.icon||"🏷️",color:v.color||"#6366F1"}]:categories.map(x=>x.id===id?{...x,...v}:x);
            await updateCategories(newCats); closeModal(); showToast(modal==="addCat"?"Added":"Updated");
          }}
      ...
      {(modal==="addStore"||modal==="editStore")&&(
        <EditItemModal title={modal==="addStore"?"New Store":"Edit Store"} fields={[{key:"label",label:"Store name",placeholder:"e.g. Loblaws"}]}
          initial={modalData?{label:modalData}:{label:""}}
          onSave={async v=>{if(!v.label?.trim())return;const n=modal==="addStore"?[...stores,v.label.trim()]:stores.map(x=>x===modalData?v.label.trim():x);await updateStores(n);closeModal();showToast(modal==="addStore"?"Added":"Updated");}}
      ...
      {(modal==="addPM"||modal==="editPM")&&(
        <EditItemModal title={modal==="addPM"?"New Payment Method":"Edit Payment Method"} fields={[{key:"label",label:"Method name",placeholder:"e.g. WeChat Pay"}]}
          initial={modalData?{label:modalData}:{label:""}}
          onSave={async v=>{if(!v.label?.trim())return;const n=modal==="addPM"?[...payMethods,v.label.trim()]:payMethods.map(x=>x===modalData?v.label.trim():x);await updatePayMethods(n);closeModal();showToast(modal==="addPM"?"Added":"Updated");}}
      ...
      {/* ── Nav bar — pure CSS, guaranteed alignment ── */}
      <nav className="nav-bar">
        {[
          {id:"home",    icon:"⊞", label:"Overview"},
          {id:"add",     icon:"+",  label:"Add",    lg:true},
          {id:"stats",   icon:"◑", label:"Stats"},
          {id:"history", icon:"≡", label:"History"},
          {id:"settings",icon:"⚙", label:"Settings"},
        ].map(t=>(
          <button key={t.id} className="nav-item" onClick={()=>setView(t.id)} style={{color:view===t.id?"#818CF8":S.muted}}>
            <span className={`nav-icon${t.lg?" lg":""}`}>{t.icon}</span>
            <span className="nav-label" style={{fontWeight:view===t.id?600:400}}>{t.label}</span>
          </button>
        ))}
      </nav>

      <input type="file" ref={fileRef}     accept="image/*" capture="environment" style={{display:"none"}} onChange={e=>{if(e.target.files[0])handlePhoto(e.target.files[0],setForm);}}/>
      <input type="file" ref={editFileRef} accept="image/*"                       style={{display:"none"}} onChange={e=>{if(e.target.files[0])handlePhoto(e.target.files[0],u=>setModalData(d=>({...d,...u})));}}/>
      <input type="file" ref={bulkFileRef} accept="image/*"                       style={{display:"none"}} onChange={e=>{if(e.target.files[0])handleBulkScan(e.target.files[0]);}}/>
    </div>
  );
}
    ...
    <div style={{background:"#0D0D0D",border:`1px solid ${settled?"#14532D":S.border2}`,borderRadius:18,padding:"18px 20px",marginBottom:12}}>
      ...
      {settled?(
        ...
        <>
          ...
          <div style={{fontSize:14,color:S.muted2,marginBottom:14}}>{allNet>0?"🟣 Tim owes Yanne":"🟢 Yanne owes Tim"}<span style={{fontSize:12,color:S.muted}}> · all time cumulative</span></div>
      ...
      {showPhoto&&(
        <div>
          <div className={`scan-zone${isAnalyzing?" shimmer":""}`} onClick={()=>fileRef.current?.click()} style={{position:"relative",overflow:"hidden"}}>
            {form.photo
              ?<><img src={form.photo} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.22}}/><div style={{position:"relative",zIndex:1,textAlign:"center"}}><div style={{fontSize:22}}>{isAnalyzing?"🔍":"✓"}</div><div style={{fontSize:12,color:"#888"}}>{isAnalyzing?"Scanning...":"Scanned · tap to change"}</div></div></>
              :<><div style={{fontSize:24}}>📷</div><div style={{fontWeight:600,fontSize:13,color:"#555"}}>Take Photo / Scan Receipt</div><div style={{fontSize:11,color:"#444"}}>AI auto-fills details</div></>
            }
          </div>
          <label className="upload-btn" style={{marginTop:8}}><input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{if(e.target.files[0])onPhoto(e.target.files[0]);}}/>🖼️ Upload from photo library</label>
        </div>
      )}
      <div>
...
function AddView({form,setForm,onSave,onPhoto,onBulkScan,bulkFileRef,fileRef,isAnalyzing,syncing,categories,stores,payMethods,S}) {
  return (
    <div className="slide-up">
      <label className="upload-btn" style={{marginBottom:14,background:"#0F172A",borderColor:"#1E3A5F",color:"#93C5FD",gap:10,height:58}}>
        <input type="file" accept="image/*" style={{display:"none"}} ref={bulkFileRef} onChange={e=>{if(e.target.files[0])onBulkScan(e.target.files[0]);}}/>
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {!record.isSettlement&&(
          <div style={{marginBottom:12}}>
            ...
            <label className="upload-btn"><input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{if(e.target.files[0])onPhoto(e.target.files[0]);}} ref={editFileRef}/>{isAnalyzing?"🔍 Scanning...":"🖼️ Upload / change photo"}</label>
          </div>
        )}
...
function StatsView({catStats,total,allNet,monthNet,filtered,filterMonth,filterYear,setFilterMonth,setFilterYear,onSettle,currency,S}) {
  ...
  const top=Object.entries(storeMap).sort((a,b)=>b[1]-a[1]).slice(0,5);
  const pmMap={}; exp.forEach(r=>{pmMap[r.paymentMethod||"Other"]=(pmMap[r.paymentMethod||"Other"]||0)+r.amount;});
  const pm=Object.entries(pmMap).sort((a,b)=>b[1]-a[1]);
  return (
    <div className="slide-up">
      <PeriodPicker filterMonth={filterMonth} filterYear={filterYear} setFilterMonth={setFilterMonth} setFilterYear={setFilterYear} S={S}/>
      <BalanceCard allNet={allNet} monthNet={monthNet} currency={currency} onSettle={onSettle} S={S}/>
          ...
          {catStats.map(c=>(
            <div key={c.id} style={{padding:"12px 16px",borderTop:`1px solid ${S.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{c.icon}</span><span style={{fontWeight:500,fontSize:14}}>{c.label}</span><span style={{fontSize:11,color:S.muted,background:S.surface2,padding:"2px 7px",borderRadius:6}}>{c.count}</span></div>
                <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:12,fontWeight:700,color:c.color}}>{total>0?Math.round((c.total/total)*100):0}%</span><span style={{fontFamily:"'DM Mono',monospace",fontWeight:700,fontSize:14}}>{fmt(c.total,currency)}</span></div>
              </div>
      ...
      {top.length>0&&<div className="card" style={{overflow:"hidden",marginBottom:12}}>
        ...
        {top.map(([s,a],i)=>(
          <div key={s} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderTop:`1px solid ${S.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{color:S.muted,fontSize:11,width:16}}>{i+1}</span><span style={{fontSize:14,fontWeight:500}}>{s}</span></div>
            <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:S.muted}}>{total>0?Math.round((a/total)*100):0}%</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:14,fontWeight:700}}>{fmt(a,currency)}</span></div>
          </div>
        ))}
      </div>}
      {pm.length>0&&<div className="card" style={{overflow:"hidden"}}>
      ...
      <div className="card" style={{overflow:"hidden"}}>
        {items.map((item,i)=>(
          <div key={i} className="row" onClick={()=>onEdit(item)} style={{cursor:"pointer"}}>
            {render(item)}<span style={{color:S.muted,fontSize:12,marginLeft:"auto"}}>Edit ›</span>
          </div>
        ))}
      </div>
    </div>
