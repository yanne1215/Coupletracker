...
// ─── Helpers ──────────────────────────────────────────────────────────────────
// Convert file to base64 + detect media type — handles screenshots with empty type
...
// Extract first JSON array or object from a string (handles markdown fences, extra text)
function extractJSON(text) {
  ...
  if (arrIdx !== -1 && (objIdx === -1 || arrIdx < objIdx)) {
    ...
    if (lastBracket > arrIdx) return JSON.parse(cleaned.slice(arrIdx, lastBracket + 1));
  }
  if (objIdx !== -1) {
    const lastBrace = cleaned.lastIndexOf("}");
    if (lastBrace > objIdx) return JSON.parse(cleaned.slice(objIdx, lastBrace + 1));
  }
...
const db = {
  async getExpenses() {
    ...
    return (await r.json()).map(row => ({
      ...
      note: row.note || "",
      payer: row.payer === "split" ? "yanne" : (row.payer || "yanne"),
  ...
  async insertMany(recs) {
    ...
    const r = await fetch(`${SUPA_URL}/rest/v1/expenses`, {
      ...
      body: JSON.stringify(recs.map(rec => ({
        ...
        amount: rec.amount, note: rec.note, payer: rec.payer,
        payment_method: rec.paymentMethod, currency: rec.currency,
        split: rec.split, is_settlement: false,
      }))),
    });
    if (!r.ok) throw new Error(await r.text());
    return await r.json();
...
export default function App() {
  ...
  // File input refs — hidden inputs triggered programmatically
  const receiptInputRef  = useRef();   // single receipt, camera
  const libraryInputRef  = useRef();   // single receipt, library
  const bulkInputRef     = useRef();   // bank statement
  const editReceiptRef   = useRef();   // photo in edit modal
  ...
  useEffect(() => {
    Promise.all([
      ...
      setLoading(false);
    }).catch(() => { setLoading(false); showToast("Could not connect to server", "err"); });
  }, []);
  ...
  // ── AI image analysis ──────────────────────────────────────────────────────
  // Single shared function — takes a File, returns AI response text
  const analyzeImage = async (file, prompt, maxTokens = 800) => {
  ...
  const handlePhoto = useCallback(async (file, setter) => {
    ...
    // Store dataUrl first so photo preview appears even if AI fails
    const { dataUrl } = await fileToBase64(file).catch(() => ({ dataUrl: null }));
    try {
      const text = await analyzeImage(
        file,
        `Analyze this receipt or shopping photo. Reply ONLY with a valid JSON object, no markdown, no extra words:\n` +
        `{"amount":number,"store":"merchant name — for Canadian stores use: Loblaws,No Frills,Metro,Sobeys,Costco,Walmart,Tim Hortons,McDonald's,A&W,Harvey's,The Keg,Canadian Tire,Home Depot,IKEA,Shoppers Drug Mart,Petro-Canada,Esso,Cineplex,Sport Chek,LCBO,Beer Store,Best Buy,Dollarama","category":"one of [${catList}]","note":"max 40 chars","currency":"CAD|USD|EUR|GBP|HKD|TWD|JPY|AUD"}`,
        700,
      );
      ...
      setter(f => ({ ...f, photo: dataUrl }));
      showToast("Could not read receipt — fill in details manually", "info");
    }
  ...
  const handleBulkScan = useCallback(async (file) => {
    ...
    try {
      const text = await analyzeImage(
        file,
        `This image is a bank statement or transaction list. Extract every purchase/debit transaction.\n` +
        `Reply ONLY with a valid JSON array, absolutely no markdown fences, no extra words:\n` +
        `[{"date":"${nowYear}-MM-DD","amount":number,"store":"merchant name","category":"one of [${catList}]","note":"max 30 chars","currency":"CAD|USD|EUR|GBP|HKD|TWD|JPY|AUD"}]\n` +
        `Rules: positive amounts only (debits/purchases), skip credits and refunds, assume year ${nowYear} if missing, return [] if nothing found.`,
        3000,
      );
      ...
      setBulkError(
        "Could not read the statement. Tips:\n" +
        "• Make sure the screenshot shows amounts and dates clearly\n" +
        "• Crop out unrelated content\n" +
  ...
  const handleBulkSave = async () => {
    ...
    setSyncing(true);
    try {
      const rows  = toSave.map(t => ({ ...t, paymentMethod: t.paymentMethod || "Credit Card" }));
  ...
  const handleSettle = async () => {
    ...
    const pLabel = allNet > 0 ? "Tim"   : "Yanne";
    const rLabel = allNet > 0 ? "Yanne" : "Tim";
    setSyncing(true);
    try {
      ...
      setRecords(r => [{ ...s, id }, ...r]);
      closeModal();
      showToast(`Settled! ${pLabel} paid ${rLabel} ${fmt(absNet, currency)}`);
    } catch (err) { console.error(err); showToast("Failed to record", "err"); }
    setSyncing(false);
  };
  ...
  const catStats = categories
    .map(c => ({ ...c, total: filtered.filter(r => !r.isSettlement && r.category === c.id).reduce((s, r) => s + r.amount, 0), count: filtered.filter(r => !r.isSettlement && r.category === c.id).length }))
  ...
  const CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .scan-zone:hover,.scan-zone:active{border-color:#6366F1;}
    ...
    .upload-btn:hover,.upload-btn:active{border-color:#6366F1;color:#818CF8;}
    ...
    .nav-icon-lg{font-size:22px;font-weight:700;line-height:1;}
  ...
  return (
    <div style={{ minHeight:"100vh", background:S.bg, fontFamily:"'DM Sans',sans-serif", color:S.text }}>
      ...
      {/* Hidden file inputs — triggered via .click() */}
      <input className="hidden-input" ref={receiptInputRef}  type="file" accept="image/*" capture="environment" onChange={e => { if (e.target.files[0]) { handlePhoto(e.target.files[0], setForm); e.target.value=""; } }} />
      <input className="hidden-input" ref={libraryInputRef}  type="file" accept="image/*"                       onChange={e => { if (e.target.files[0]) { handlePhoto(e.target.files[0], setForm); e.target.value=""; } }} />
      <input className="hidden-input" ref={bulkInputRef}     type="file" accept="image/*"                       onChange={e => { if (e.target.files[0]) { handleBulkScan(e.target.files[0]); e.target.value=""; } }} />
      <input className="hidden-input" ref={editReceiptRef}   type="file" accept="image/*"                       onChange={e => { if (e.target.files[0]) { handlePhoto(e.target.files[0], u => setModalData(d => ({ ...d, ...u }))); e.target.value=""; } }} />
      ...
      {modal === "bulkReview" && (
        <div className="overlay">
          <div className="modal">
            ...
            <div style={{ fontSize:12, color:S.muted, marginBottom:14 }}>Tap a row to include/exclude · edit fields inline</div>
            ...
            {!isAnalyzing && bulkError && (
              <div style={{ background:"#3B0000", border:"1px solid #7F1D1D", borderRadius:12, padding:"16px", marginBottom:16 }}>
                <div style={{ fontWeight:600, color:"#FCA5A5", marginBottom:8 }}>⚠️ Could not extract transactions</div>
                <div style={{ fontSize:12, color:"#FCA5A5", opacity:0.85, whiteSpace:"pre-line" }}>{bulkError}</div>
                <button className="btn upload-btn" style={{ marginTop:14, borderColor:"#7F1D1D", color:"#FCA5A5" }} onClick={() => bulkInputRef.current?.click()}>
                  Try another screenshot
                </button>
              </div>
            )}

            {!isAnalyzing && bulkItems && bulkItems.length === 0 && !bulkError && (
            ...
            {!isAnalyzing && bulkItems && bulkItems.length > 0 && (
              <>
                ...
                <button className="btn" onClick={handleBulkSave} disabled={syncing} style={{ width:"100%", marginTop:4, padding:"15px", background:syncing?"#3730A3":"#6366F1", borderRadius:13, fontSize:15, fontWeight:700, color:"#fff" }}>
                  {syncing ? "Saving..." : `Save ${bulkItems.filter(i => i.selected).length} Expense${bulkItems.filter(i=>i.selected).length !== 1?"s":""}`}
                </button>
              </>
            )}
          </div>
      ...
      {modal === "editRecord" && modalData && (
        <EditRecordModal
          ...
          onPhotoClick={() => editReceiptRef.current?.click()}
          S={S}
        />
      )}
      ...
      {(modal === "addCat" || modal === "editCat") && (
        <EditItemModal
          ...
          onSave={async v => {
            ...
            const arr = modal === "addCat" ? [...categories, { id, label:v.label.trim(), icon:v.icon||"🏷️", color:v.color||"#6366F1" }] : categories.map(x => x.id === id ? { ...x, ...v } : x);
          ...
          onDelete={modal === "editCat" ? async () => {
            ...
            await updateCategories(categories.filter(x => x.id !== modalData.id)); closeModal(); showToast("Deleted","info");
          } : null}
      ...
      {(modal === "addPM" || modal === "editPM") && (
        <EditItemModal
          ...
          onDelete={modal === "editPM" ? async () => {
            ...
            await updatePayMethods(payMethods.filter(x => x !== modalData)); closeModal(); showToast("Deleted","info");
          } : null}
          onClose={closeModal} S={S}
        />
      )}
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"16px 14px 74px" }}>
        {loading ? (
          ...
          <>
            ...
            {view==="add"      && <AddView      form={form} setForm={setForm} onSave={handleSave} onCameraClick={()=>receiptInputRef.current?.click()} onLibraryClick={()=>libraryInputRef.current?.click()} onBulkClick={()=>bulkInputRef.current?.click()} isAnalyzing={isAnalyzing} syncing={syncing} categories={categories} stores={stores} payMethods={payMethods} S={S} />}
      ...
      <nav className="nav-bar">
        {[
          ...
          <button key={t.id} className="nav-item" onClick={() => setView(t.id)} style={{ color: view===t.id ? "#818CF8" : S.muted }}>
            <span className={t.lg ? "nav-icon-lg" : "nav-icon"}>{t.icon}</span>
      ...
      {settled ? (
        ...
        <>
          ...
          <div style={{ fontSize:14, color:S.muted2, marginBottom:14 }}>
            ...
            <span style={{ fontSize:12, color:S.muted }}> · all time cumulative</span>
          </div>
  ...
  if (record.isSettlement) return (
    <div className="row" onClick={() => onEdit(record)} style={{ background:"#071A0F" }}>
      ...
      <div style={{ textAlign:"right" }}>
        ...
        <div style={{ fontSize:10, color:"#4ADE80" }}>settlement</div>
      </div>
    </div>
  );
...
function EditRecordModal({ record, categories, stores, payMethods, onSave, onDelete, onClose, isAnalyzing, onPhotoClick, S }) {
  ...
  const [showConfirm, setShowConfirm] = useState(false);  // FIX: was broken ternary
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:"flex", gap:10, marginTop:16 }}>
          {showConfirm ? (
            ...
            <>
              <button className="btn" onClick={() => setShowConfirm(false)} style={{ flex:1, padding:"13px", borderRadius:11, background:S.surface2, fontSize:14, color:S.muted2 }}>Cancel</button>
            ...
            // Normal state
            <>
              <button className="btn" onClick={() => setShowConfirm(true)} style={{ padding:"13px 16px", borderRadius:11, background:"#1C0A0A", color:"#FCA5A5", fontWeight:600, fontSize:14, border:"1px solid #7F1D1D" }}>Delete</button>
