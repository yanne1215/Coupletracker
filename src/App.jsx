          ...
          body: JSON.stringify({
            ...
            messages: [{
              ...
              content: [
                ...
                { type: "text", text: `Analyze this receipt or shopping photo. Extract the following and respond ONLY with a JSON object, no other text:
{
  "amount": number (total amount in CAD, 0 if not found),
  "store": "store name" (identify the Canadian retailer - e.g. Loblaws, Costco, Canadian Tire, Tim Hortons, Shoppers Drug Mart, No Frills, Metro, Sobeys, Winners, HomeSense, The Bay, Sport Chek, Best Buy, Dollarama, LCBO, Beer Store, Esso, Petro-Canada, Shell, McDonald's, A&W, Harvey's, Swiss Chalet, Boston Pizza, Cineplex, etc. If unsure write the store name from the receipt.),
  "category": "groceries|dining|transport|shopping|entertainment|travel|health|utilities|home|other",
    ...
    <div style={{ minHeight: "100vh", background: "#F5F4F0", fontFamily: "'DM Sans', sans-serif", color: "#1A1A1A" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card { background: #fff; border-radius: 14px; border: 1px solid #E8E6E1; }
        input, select, textarea { font-family: 'DM Sans', sans-serif; background: #fff; border: 1.5px solid #E8E6E1; border-radius: 10px; color: #1A1A1A; padding: 11px 14px; width: 100%; font-size: 15px; outline: none; transition: border-color 0.15s; }
        ...
        .toast-wrap { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; animation: toastIn 0.2s ease; white-space: nowrap; }
        @keyframes toastIn { from { opacity:0; transform: translateX(-50%) translateY(-8px); } to { opacity:1; transform: translateX(-50%) translateY(0); } }
        .nav-item { display: flex; flex-direction: column; align-items: center; gap: 3px; cursor: pointer; border: none; background: transparent; padding: 8px 0; flex: 1; transition: color 0.15s; }
        .scan-zone { border: 2px dashed #D4D0C8; border-radius: 12px; background: #FAFAF8; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; height: 130px; cursor: pointer; transition: all 0.15s; }
        .scan-zone:hover { border-color: #2563EB; background: #F0F5FF; }
        .progress-bar { height: 6px; background: #F0EDE8; border-radius: 3px; overflow: hidden; }
        .progress-fill { height: 100%; border-radius: 3px; transition: width 0.5s ease; }
        .shimmer { background: linear-gradient(90deg, #f0ede8 25%, #e8e5e0 50%, #f0ede8 75%); background-size: 200% 100%; animation: shimmer 1.4s linear infinite; }
        @keyframes shimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
        .modal-bg { position: fixed; inset: 0; background: rgba(0,0,0,0.35); z-index: 800; display: flex; align-items: flex-end; justify-content: center; animation: fadeIn 0.2s ease; }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        .modal-box { background: #fff; border-radius: 20px 20px 0 0; padding: 24px; width: 100%; max-width: 480px; animation: slideModal 0.25s ease; }
